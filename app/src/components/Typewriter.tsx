// @ts-nocheck
// Vendored from Originkit "typewriter" (real source via MCP).
// Patches: framer-motion cursor blink -> CSS animation (repo has no framer-motion);
// RenderTarget shimmed for a non-Framer runtime.
"use client";

import * as React from "react"
import { useEffect, useState } from "react"
type AnimationOptions = any; type Variants = any;
const RenderTarget: any = { current: () => "preview", export: "export", thumbnail: "thumbnail", canvas: "canvas" };
                    ...
                    (isStatic || isCanvas ? (
                        ...
                        <span
                            style={{
                                ...
                                marginLeft: "0.25rem",
                                visibility: cursorHidden ? "hidden" : "visible",
                                animation: "twBlink 1.06s steps(2, jump-none) infinite",
                            }}
                        >
                            {cursorChar}
                            <style>{`@keyframes twBlink { 0% { opacity: 1; } 50% { opacity: 0; } }`}</style>
                        </span>
                    ))}
            </div>
        </div>
