// @ts-nocheck
// Vendored from Originkit "particlesphere" (real source via MCP), reworked into a
// particle FOOTBALL: per-instance colors paint 12 pentagon patches at icosahedron
// vertices (the truncated-icosahedron construction). CDN three -> local package,
// RenderTarget shimmed, reduced-motion stops auto-rotation.
// Delivered by Originkit · stack: nextjs · styling: css
...
const RenderTarget = { current: () => "preview", canvas: "canvas", export: "export", thumbnail: "thumbnail" }
import {
    Scene,
    PerspectiveCamera,
    WebGLRenderer,
...
export default function ParticleSphereRefactor({
    ...
    useEffect(() => {
        ...
        // Football pattern: 12 pentagon patches centred on the vertices of an
        // icosahedron (how a real truncated-icosahedron ball is built).
        const PHI = (1 + Math.sqrt(5)) / 2
        const ICOSA = [
            [0, 1, PHI], [0, -1, PHI], [0, 1, -PHI], [0, -1, -PHI],
            [1, PHI, 0], [-1, PHI, 0], [1, -PHI, 0], [-1, -PHI, 0],
            [PHI, 0, 1], [-PHI, 0, 1], [PHI, 0, -1], [-PHI, 0, -1],
        ].map(([a, b, c]) => new Vector3(a, b, c).normalize())
        const PENTAGON_ANGLE = 0.40
        const pentagonColorObj = new Color(resolveTokenColor(pentagonColor) || pentagonColor)
        const patternFlags: boolean[] = []
        ...
        for (let i = 0; i < particlesCount; i++) {
            ...
            patternFlags.push(ICOSA.some((c) => new Vector3(x, y, z).angleTo(c) < PENTAGON_ANGLE))
        ...
        if (particleShape === "sphere") {
            ...
            // Per-instance colors via the proper InstancedMesh API — a plain
            // Float32BufferAttribute is not instanced and renders uniform.
            for (let i = 0; i < particlesCount; i++) {
                particles.setColorAt(i, patternFlags[i] ? pentagonColorObj : baseColorObj)
            }
            ...
            for (let i = 0; i < particlesCount; i++) {
                const idx = i * 3
                const pc2 = patternFlags[i] ? pentagonColorObj : baseColorObj
                colors[idx] = pc2.r
                colors[idx + 1] = pc2.g
                colors[idx + 2] = pc2.b
            }
